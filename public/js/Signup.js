$(document).ready(function() {
    ...
    $("[name='username']").popover({trigger: 'focus', content: "" ,placement : 'top'});            
    $("[name='username']").keyup(function() {
        ...
        if (username.length>30) {
            ...
            $(this).attr("data-content", "Please use only letters, numbers and underscore");  
            $(this).popover('show');   
            $("#username").addClass("has-error");
            $("#username .glyphicon-remove").show();
            $("#username").removeClass("has-success"); 
    ...
    $("[name='username']").focusout(function(){ 
        ...
        if (username.trim()=="") { 
            ...
            $.get("/api/user/checkUsername/" + username).then(function(data, status){
                if (data && data.count) {
                    if (data.count !== "0") {
                        $("[name='username']").attr("data-content", "Username already exists. Try another.");   
                        $("#username").addClass("has-error");
                        $("#username .glyphicon-remove").show(); 
                        $("#username").removeClass("has-success"); 
                        $("#username .glyphicon-ok").hide();      
    ...
    $("[name='password']").popover({trigger: 'focus', content: "" ,placement : 'top'});            
    ...
    $("[name='repassword']").popover({trigger: 'focus', content: "" ,placement : 'top'});            
    ...
    $("[name='name']").popover({trigger: 'focus', content: "" ,placement : 'top'});            
    $("[name='name']").keyup(function() {
        ...
        else if (!/^[a-zA-Z\sÀÁÂÃÈÉÊÌÍÒÓÔÕÙÚĂĐĨŨƠàáâãèéêìíòóôõùúăđĩũơƯĂẠẢẤẦẨẪẬẮẰẲẴẶẸẺẼỀỀỂưăạảấầẩẫậắằẳẵặẹẻẽềềểỄỆỈỊỌỎỐỒỔỖỘỚỜỞỠỢỤỦỨỪễệỉịọỏốồổỗộớờởỡợụủứừỬỮỰỲỴÝỶỸửữựỳỵỷỹ]*$/.test(name)) {
    ...
    $("[name='email']").popover({trigger: 'focus', content: "" ,placement : 'top'});  
    ...
    $("[name='email']").focusout(function(){
        ...
        if (! /^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,3})+$/.test(email)) { 
            ...
            $.get("/api/user/checkEmail/" + email).then((data, status) => {
                if (data && data.count) {
                    if (data.count !== "0") {
                        $("[name='email']").attr("data-content", "Email already exists. Try another.");   
                        $("#email").addClass("has-error");
                        $("#email .glyphicon-remove").show(); 
                        $("#email").removeClass("has-success"); 
                        $("#email .glyphicon-ok").hide();      
    ...
    $("[name='phonenumber']").popover({trigger: 'focus', content: "" ,placement : 'top'});            
    ...
    $("[name='phonenumber']").focusout(function(){  
        $(this).popover('hide');  
        const phonenumber = $(this).val();  
        if (!(phonenumber.length == 10 || phonenumber.length == 11 || phonenumber.trim() == "")) { 
    ...
    $("[name='birthday']").popover({trigger: 'focus', content: "" ,placement : 'top'}); 
    ...
    $("#btn-signup").click(function() {
        if ($("[name='username']").val().trim() == "" 
            ...
            $("#signupalert").show();
            if ($("[name='username']").val().trim() == ""  && !$("#username").hasClass("has-error")) {   
                $("#username").addClass("has-error");
                $("#username .glyphicon-remove").show();
                $("[name='username']").attr("data-content", "You can't leave this empty.");  
            }
            if ($("[name='password']").val().trim() == "" && !$("#password").hasClass("has-error")) {   
                $("#password").addClass("has-error");
                $("#password .glyphicon-remove").show();
                $("[name='password']").attr("data-content", "You can't leave this empty.");  
            }
            if ($("[name='repassword']").val().trim() == "" && !$("#repassword").hasClass("has-error")) {   
                $("#repassword").addClass("has-error");
                $("#repassword .glyphicon-remove").show();
                $("[name='repassword']").attr("data-content", "You can't leave this empty.");  
            }
            if ($("[name='name']").val().trim() == "" && !$("#name").hasClass("has-error")) {   
                $("#name").addClass("has-error");
                $("#name .glyphicon-remove").show();
                $("[name='name']").attr("data-content", "You can't leave this empty.");  
            }
            if ($("[name='email']").val().trim() == "" && !$("#email").hasClass("has-error")) {   
                $("#email").addClass("has-error");
                $("#email .glyphicon-remove").show();
                $("[name='email']").attr("data-content", "You can't leave this empty.");  
            }
        }   
        else if (!($("#username").hasClass("has-success")
            ...
            && !$("#phonenumber").hasClass("has-error")) )
        {
            ...
            if ($(".checkbox input").prop("checked")) {
                ...
                $("#signupalert p").text("Do you agree the rules below? Please tick the checkbox!");
                $("#signupalert").show();
            }
