function checkAndChange(){
    ...
    if(oldPass.trim()=="" || newPass.trim()=="" || renewPass.trim()==""){
        ...
        if( oldPass.trim() == "")
            ...
            $("[name='reNewPass']").focus();
    }
    else{
        $("#loginalert").hide();
        if(!(newPass.toString() == renewPass.toString())){
            $("#loginalert p").text("Error: your new pass and retype new pass are incorrect.");
            $("#loginalert").show();
        }
        else{
            $("#loginalert").hide();
            ...
            $.ajax({
                ...
                url: '/api/changePass', // chưa xong phần này tạo thêm file js để check
