getTime = date => {
    ...
    return preDate + date.getDate() + "/" + preMonth + (date.getMonth()+1) + "/" + date.getFullYear();
}
        ...
        success: function(result) {
            if(result.success) {
                ...
                if (localStorage.getItem("username") !== username) {
                    $.ajax({
                        type: 'GET',
                        url: `/api/follow/checkFollow/${id1}/${id2}` , 
                        headers: {
                            authorization: localStorage.getItem('access_token')? ("Bearer " +  localStorage.getItem('access_token')) : undefined
                        },
                        success: (data) => {
                            if(data.success) {
                                ...
                                if(!following)
                                    $('#follow').html(`<button type="button" class="btn btn-info">Follow me</button>`);
                                else 
                                    $('#follow').html(`
                ...
                $.ajax({
                    type: 'GET',
                    url: '/api/post/getOfUser/' + user._id,
                    headers: {
                        authorization: localStorage.getItem('access_token')? ("Bearer " +  localStorage.getItem('access_token')) : undefined
                    },
                    success: (data) => {
                        ...
                        if (data.post) {
                            if (data.post.length == 0) {
                                ...
                                for (let i=0; i<data.post.length; i++) showPost(data.post[i]); 
                            }
                        }
                    }
                });
