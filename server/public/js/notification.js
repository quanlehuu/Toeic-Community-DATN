getDateTime = date => {
    ...
    return date.getHours() + ":" + preMin + date.getMinutes() + " " + preDate + date.getDate() + "/" + preMonth + (date.getMonth()+1) + "/" + date.getFullYear();
}
            ...
            if (data.noti && data.noti.length !== 0) {
                ...
                for (i=0; i<notis.length; i++) {
                    ...
                    if (noti.posted) {
                        ...
                        $("#" + id + " .notification-detail span").text(noti.follower.username + " posted"); 
                        $("#" + id + " .notification-detail i").text(getDateTime(new Date(noti.time))); 
                    }
                    else {
                        $("#" + id + " a").attr("href","/user="+ noti.follower.username); 
                        $("#" + id + " img").attr("src", noti.follower.img); 
                        $("#" + id + " .notification-detail span").text(noti.follower.username + " followed you"); 
                        $("#" + id + " .notification-detail i").text(getDateTime(new Date(noti.time)));
                    }
                ...
                if (num == 0) $(".notis").append("<p style='font-size:16px; text-align:center; color: black;'>Don't have any notifications!</p>");
    ...
    $('.notification .fa-bell').on('click', function () {
        ...
        if (sideBar.hasClass("active")) {
            ...
            $(".notification-view").scroll(function() {
                if ($(this).scrollTop() == $(" .notis").height() - $(this).height() + 40) {
                    num=num+1;
                    getNoti(num);
                }
            });
