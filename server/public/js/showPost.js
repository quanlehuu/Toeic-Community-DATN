            ...
            success: (data) => {
                if (data.comment) {
                    ...
                    if (data.comment.length == 0) {
                        $("#" + id + " .modal-body").append("<div style='text-align: center;'>Don't have any comments!</div>");
                    }
                    else {
                        ...
                        for (i=0; i<comment.length; i++) { 
                            ...
                            $("#" + commentId + " a").attr("href","/user="+ comment[i].ofUser.username); 
                            $("#" + commentId + " .user-cmt i").text(getTime(new Date(comment[i].time)));
                            $("#" + commentId + " span").text(comment[i].content);
                            $.ajax({
                                type: 'GET',
                                contentType: 'application/json',
                                url: '/api/voteComment/' + commentId,
                                headers: {
                                    authorization: localStorage.getItem('access_token')? ("Bearer " +  localStorage.getItem('access_token')) : undefined
                                },						
                                success: (data) => {
